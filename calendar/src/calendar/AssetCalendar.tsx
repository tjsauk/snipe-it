...
function clampEditPeriodWithinAsset(
  ...
  return normalizePeriod(start, endExclusive);
}

function normalizePeriodsKey(periods: TimePeriod[]) {
  return normalizePeriodsForKey(periods);
}

function earliestEndOfPeriods(periods: TimePeriod[]) {
  return Math.min(...periods.map((p) => new Date(p.end).getTime()));
}
...
function applyGroupLimitToSelections(
  ...
  const groupMap = new Map<string, { periods: TimePeriod[]; assetIds: string[]; earliestEnd: number }>();
  ...
  for (const asset of assets) {
    ...
    if (existing) {
      ...
      groupMap.set(key, { periods, assetIds: [asset.id], earliestEnd: earliestEndOfPeriods(periods) });
    }
  }
  ...
  if (groups.length <= MAX_VISIBLE_GROUPS) {
    return { byAsset, wasLimited: false };
  }

  const cutoffPeriods = groups[MAX_VISIBLE_GROUPS - 1].periods;
  const keepKeys = new Set(groups.slice(0, MAX_VISIBLE_GROUPS).map((g) => normalizePeriodsKey(g.periods)));
  ...
  return { byAsset: nextByAsset, wasLimited: true };
}
...
function layoutDraftColumns(dayBars: InternalDayBar[]): InternalPositionedDayBar[] {
  ...
  const columns = [...byColumnKey.entries()].map(([key, bars]) => ({
    key,
    bars,
    earliestEnd: Math.min(...bars.map((b: InternalDayBar) => b.sortEnd)),
  }));
  ...
  columns.forEach((column, index) => {
    ...
    column.bars.forEach((bar: InternalDayBar) => {
      positioned.push({ ...bar, leftPct, widthPct });
    });
  });
...
function layoutExistingColumns(dayBars: InternalDayBar[]): InternalPositionedDayBar[] {
  ...
  const sorted = [...dayBars].sort((a, b) => {
    ...
    return s !== 0 ? s : a.end.getTime() - b.end.getTime();
  });
...
export default function AssetCalendar({
  ...
  const [periods, setPeriods] = useState<SelectionPeriodDef[]>([{ id: 'p1', requestedRange: null }]);
  ...
  const [editPeriodsByAssetUser, setEditPeriodsByAssetUser] = useState<Record<string, Record<string, TimePeriod[]>>>({});
  ...
  const activePeriod = periods.find((p: SelectionPeriodDef) => p.id === activePeriodId) ?? periods[0] ?? null;
  ...
  const draftsByAssetByPeriodId = useMemo(() => {
    ...
    for (const period of periods as SelectionPeriodDef[]) {
      const range =
        mode === 'checkout'
          ? period.requestedRange ? { start: formatDateTime(minSelectableStart), end: period.requestedRange.end } : null
          : period.requestedRange;

      if (!range) { result[period.id] = {}; continue; }

      const safeRange = clampRangeToMinStart(range, minSelectableStart);
      if (!safeRange) { result[period.id] = {}; continue; }

      const byAsset: Record<string, TimePeriod[]> = {};
      for (const asset of visualAssets) {
        byAsset[asset.id] = buildPeriodsForAsset(safeRange, asset.existingPeriods, primaryUser, input.continuousCutMode ?? true);
      }
  ...
  const selectionGroupLimitMessage = useMemo(() => {
    ...
    for (const period of periods as SelectionPeriodDef[]) {
      ...
      if (applyGroupLimitToSelections(byAsset, visualAssets).wasLimited) {
    ...
    setPeriods((prev) => {
      ...
      if (hasAnyRange) {
        return prev.map((p: SelectionPeriodDef, index: number) =>
          index === 0 && p.requestedRange === null ? { ...p, requestedRange: currentHourPeriod } : p,
        );
      }
      return prev.map((p: SelectionPeriodDef, index: number) =>
  ...
  const updatePeriodRange = (periodId: string, nextRange: TimePeriod | null) => {
    setPeriods((prev) =>
      prev.map((p: SelectionPeriodDef) => p.id === periodId ? { ...p, requestedRange: nextRange } : p),
    );
  };

  const updateEditPeriodRange = (assetId: string, userName: string, index: number, nextRange: TimePeriod | null) => {
    setEditPeriodsByAssetUser((prev) => {
      ...
      if (nextRange === null) {
        ...
        return { ...prev, [assetId]: assetBlock };
      }

      const clamped = clampEditPeriodWithinAsset(assetBlock, blocked, userName, index, nextRange);
  ...
  const resetActivePeriod = () => {
    if (mode === 'edit') {
      ...
      updateEditPeriodRange(selectedEditPeriod.assetId, selectedEditPeriod.userName, selectedEditPeriod.index, null);
      setSelectedEditPeriod(null);
      return;
    }
    ...
    if (mode === 'reserve') updatePeriodRange(activePeriod.id, null);
  };

  const beginDrag = (kind: DragKind, event: React.MouseEvent, periodId: string, dragRange?: TimePeriod) => {
    ...
    setDragState({ kind, startMouseHour: absoluteHour, originalRange: baseRange, periodId });
  };
  ...
  const applyRepeat = () => {
    ...
    for (let i = 1; i <= repeatCount; i++) {
      ...
      nextItems.push({
        id: `p${periods.length + nextItems.length + 1}`,
        requestedRange: {
  ...
  const removePeriod = (periodId: string) => {
    setPeriods((prev) => {
      if (prev.length <= 1) {
        return prev.map((p: SelectionPeriodDef) => p.id === periodId ? { ...p, requestedRange: null } : p);
      }
    ...
    if (dragState?.periodId === periodId) setDragState(null);
  };
  ...
  const existingDayBars = useMemo(() => {
    ...
    if (mode === 'edit') {
      for (const asset of visualAssets) {
        ...
        blocked.forEach((reservation, reservationIndex) => {
          const parts = splitPeriodForWeek({ start: reservation.start, end: reservation.end }, weekStart);
          ...
          parts.forEach((part, partIndex) => {
            ...
            result.push({
              ...
              top: rect.top, height: rect.height, color: '#9ca3af',
              label: `${asset.name} — ${reservation.userName}`, tooltip, sortEnd: rect.end.getTime(),
            });
          });
        });
      }
    ...
    for (const group of existingGroups) {
      ...
      const overlapsActive =
        ...
        group.entries.some(({ reservation }: { reservation: ExistingReservation }) => {
      ...
      const label = group.entries.length === 1 ? group.entries[0].asset.name : `${group.entries.length} reservations`;
      const tooltip = group.entries
        .map(({ asset, reservation }: { asset: Asset; reservation: ExistingReservation }) =>
  ...
  const draftDayBars = useMemo(() => {
    if (mode === 'edit') {
      ...
      visualAssets.forEach((asset: Asset) => {
        ...
        editableUsers.forEach((userName) => {
          (userBlock[userName] ?? []).forEach((period: TimePeriod, periodIndex: number) => {
            ...
            parts.forEach((part, partIndex) => {
              ...
              result.push({
                ...
                type: 'draft', dayIndex: rect.dayIndex, start: rect.start, end: rect.end,
                top: rect.top, height: rect.height, color: asset.color ?? '#2563eb',
                label: `${asset.name} — ${userName}`, tooltip, sortEnd: rect.end.getTime(), status: 'neutral',
              });
            });
          });
        });
    ...
    draftGroups.forEach((group, groupIndex) => {
      ...
      const tooltip = `Assets:\n${assetNames.join('\n')}\n\nPeriods:\n${group.periods.map((p: TimePeriod) => `${p.start} -> ${p.end}`).join('\n')}`;

      group.periods.forEach((period: TimePeriod, periodIndex: number) => {
        splitPeriodForWeek(period, weekStart).forEach((part, partIndex) => {
          const rect = periodToDisplayRect(part, weekStart);
          result.push({
            id: `draft-group-${group.periodId}-${groupIndex}-${periodIndex}-${partIndex}`,
            periodId: group.periodId, columnKey: group.groupKey,
            type: 'draft', dayIndex: rect.dayIndex, start: rect.start, end: rect.end,
            top: rect.top, height: rect.height,
            color: group.status === 'good' ? '#16a34a' : group.status === 'warning' ? '#ca8a04' : group.status === 'error' ? '#dc2626' : '#94a3b8',
            label, tooltip, sortEnd: rect.end.getTime(), status: group.status,
          });
        });
      });
    });
  ...
  const dayLayouts = useMemo(() => {
    return Array.from({ length: 7 }, (_, dayIndex) => ({
      drafts: layoutDraftColumns(draftDayBars.filter((b: InternalDayBar) => b.dayIndex === dayIndex)),
      existing: layoutExistingColumns(existingDayBars.filter((b: InternalDayBar) => b.dayIndex === dayIndex)),
    }));
  ...
  const nowMarker = useMemo(() => {
    ...
    const dayIndex = Math.floor((startOfDay(now).getTime() - startOfDay(weekStart).getTime()) / (24 * 60 * 60 * 1000));
  ...
  useEffect(() => {
    if (repeatEveryWeeks < minRepeatWeeks) setRepeatEveryWeeks(minRepeatWeeks);
    ...
    return visualAssets.map((asset: Asset) => {
      ...
      const editablePeriods = editableUsers.flatMap((userName) =>
        (editPeriodsByAssetUser[asset.id]?.[userName] ?? []).map((p: TimePeriod) => ({
          start: p.start, end: p.end, userName,
        })),
      );
      return { ...asset, existingPeriods: [...blocked, ...editablePeriods] };
    });
  ...
  const handleCellClick = (date: Date) => {
    ...
    if (mode === 'edit') {
      ...
      setEditPeriodsByAssetUser((prev) => {
        ...
        const blocked = getBlockedReservationsForAsset(asset, editableUsers);
        const clamped = clampEditPeriodWithinAsset(assetBlock, blocked, addForTarget.userName, newIndex, proposed);
        if (!clamped) return prev;
        userPeriods[newIndex] = clamped;
        assetBlock[addForTarget.userName] = userPeriods;
        return { ...prev, [addForTarget.assetId]: assetBlock };
      });

      const newIndex = editPeriodsByAssetUser[addForTarget.assetId]?.[addForTarget.userName]?.length ?? 0;
      setSelectedEditPeriod({ assetId: addForTarget.assetId, userName: addForTarget.userName, index: newIndex });
      setAddForTarget(null);
      return;
    }
    ...
    if (mode === 'checkout') {
      ...
      updatePeriodRange(activePeriod.id, { start: formatDateTime(start), end: formatDateTime(addMinutes(addHours(end, 1), -1)) });
      return;
    }
    ...
    if (clickedHour < currentStart) {
      updatePeriodRange(activePeriod.id, { start: formatDateTime(clickedHour), end: formatDateTime(currentEnd) });
    } else if (clickedHour > currentEnd) {
      updatePeriodRange(activePeriod.id, { start: formatDateTime(currentStart), end: formatDateTime(addMinutes(addHours(clickedHour, 1), -1)) });
    }
  };
  ...
  useEffect(() => {
    ...
    const onMove = (event: MouseEvent) => {
      ...
      const dayIndex = Math.max(0, Math.min(6, Math.floor(x / colWidth)));
      const hourIndex = Math.max(0, Math.min(23, Math.floor(y / HOUR_ROW_PX)));
      const deltaHours = clampHourIndex(dayIndex * 24 + hourIndex) - dragState.startMouseHour;

      let nextRange = dragState.originalRange;
      if (dragState.kind === 'move') nextRange = buildMovedRange(dragState.originalRange, deltaHours);
      else if (dragState.kind === 'resize-start') nextRange = buildResizeStartRange(dragState.originalRange, deltaHours);
      else if (dragState.kind === 'resize-end') nextRange = buildResizeEndRange(dragState.originalRange, deltaHours);

      if (mode === 'checkout') {
        nextRange = { start: formatDateTime(minSelectableStart), end: nextRange.end };
      }
    ...
    <section className="calendar-card standalone-calendar">
      <div className="calendar-toolbar calendar-toolbar-main">
        <div className="toolbar-left">
          <button type="button" onClick={() => setAnchorDate((prev) => viewMode === 'week' ? addDays(prev, -7) : new Date(prev.getFullYear(), prev.getMonth() - 1, 1))}>Prev</button>
          <button type="button" onClick={() => setAnchorDate(new Date())}>Today</button>
          <button type="button" onClick={() => setAnchorDate((prev) => viewMode === 'week' ? addDays(prev, 7) : new Date(prev.getFullYear(), prev.getMonth() + 1, 1))}>Next</button>
          <select value={viewMode} onChange={(e) => setViewMode(e.target.value as CalendarViewMode)}>
            <option value="week">Week view</option>
            <option value="month">Month view</option>
          </select>
          {mode === 'reserve' && <button type="button" onClick={addNewPeriod}>Add new</button>}
          {mode !== 'view' && <button type="button" onClick={resetActivePeriod}>{mode === 'edit' ? 'Remove selected' : 'Cancel selection'}</button>}
        </div>
        ...
        {mode === 'reserve' && activePeriod?.requestedRange && (
          <div className="toolbar-right repeat-inline">
            ...
            <label>every<input type="number" min={minRepeatWeeks} step={1} value={repeatEveryWeeks} onChange={(e) => setRepeatEveryWeeks(Math.max(minRepeatWeeks, Number(e.target.value) || minRepeatWeeks))} />weeks</label>
            <label>times<input type="number" min={1} step={1} value={repeatCount} onChange={(e) => setRepeatCount(Math.max(1, Number(e.target.value) || 1))} /></label>
            <button type="button" onClick={applyRepeat}>Apply</button>
          </div>
        )}

        {mode !== 'view' && onConfirm && (
          <div className="toolbar-right">
            <button type="button" onClick={() => onConfirm(output)}>Confirm</button>
          </div>
        )}
      </div>
          ...
          <div className="period-chip-list">
            {visualAssets.flatMap((asset: Asset) =>
              editableUsers.map((userName) => {
                const active = addForTarget?.assetId === asset.id && addForTarget?.userName === userName;
                return (
                  <button key={`${asset.id}-${userName}`} type="button" className={`period-chip ${active ? 'active' : ''}`} onClick={() => toggleAddForTarget(asset.id, userName)}>
                    <span className="period-status-dot" style={{ backgroundColor: asset.color ?? '#2563eb' }} />
                    <span className="period-chip-meta">{active ? `Click calendar for ${asset.name} — ${userName}` : `Add for ${asset.name} — ${userName}`}</span>
                  </button>
                );
              }),
            )}
        ...
        <div className="period-panel">
          <div className="period-chip-list">
            {periods.map((period: SelectionPeriodDef) => (
              <div key={period.id} className={`period-chip ${period.id === activePeriodId ? 'active' : ''} status-${periodStatuses[period.id]}`}>
                <button type="button" className="period-chip-main" onClick={() => setActivePeriodId(period.id)}>
                  <span className="period-status-dot" />
                  <span className="period-chip-meta">{dateLabel(period)}</span>
                </button>
                {periods.length > 1 && (
                  <button type="button" className="period-chip-remove" onClick={() => removePeriod(period.id)}>×</button>
                )}
              </div>
            ))}
          </div>
      ...
      {viewMode === 'month' ? (
        <MonthOverview assets={monthAssets} anchorDate={anchorDate} draftGroups={draftGroups} mode={mode} onOpenWeek={(date) => { setAnchorDate(date); setViewMode('week'); }} />
      ) : (
        <div className="week-shell" ref={gridRef}>
          <div className="week-header-spacer" />
          {weekDays.map((day) => (
            <div key={day.toISOString()} className="week-day-header">{formatDayHeader(day)}</div>
          ))}
          <div className="time-column">
            {HOURS.map((hour) => (
              <div key={hour} className="time-cell">{String(hour).padStart(2, '0')}:00</div>
            ))}
          </div>

          {weekDays.map((day, dayIndex) => (
            <div key={day.toISOString()} className={`day-column ${dayIndex % 2 === 1 ? 'alt' : ''}`}>
              {HOURS.map((hour) => (
                <button
                  type="button"
                  key={`${day.toISOString()}-${hour}`}
                  className={`hour-cell ${addHours(day, hour) < minSelectableStart && mode !== 'view' && mode !== 'edit' ? 'disabled-past' : ''}`}
                  onClick={() => handleCellClick(addHours(day, hour))}
                />
              ))}

              {nowMarker && nowMarker.dayIndex === dayIndex && (
                <div className="now-line" style={{ top: nowMarker.top }}><div className="now-dot" /></div>
              )}

              {dayLayouts[dayIndex].drafts.map((bar: InternalPositionedDayBar) => {
                const isEditBar = mode === 'edit';
                const activeRequestedStart = !isEditBar && activePeriod?.requestedRange ? parseDateTime(activePeriod.requestedRange.start) : null;
                const startVisible = isEditBar ? true : !!activeRequestedStart && activeRequestedStart >= weekStart && activeRequestedStart <= weekEnd;
                const selectedEdit = isEditBar && selectedEditPeriod ? `${selectedEditPeriod.assetId}__${selectedEditPeriod.userName}__${selectedEditPeriod.index}` : null;
                const currentBarSelected = isEditBar ? bar.periodId === selectedEdit : bar.periodId === activePeriodId;

                const showTopHandle = currentBarSelected && (isEditBar ? true : !!activePeriod?.requestedRange && bar.start.getTime() === parseDateTime(activePeriod.requestedRange.start).getTime());

                const relevantBars = dayLayouts.flatMap((layout) => layout.drafts).filter((b: InternalPositionedDayBar) =>
                  isEditBar ? b.periodId === bar.periodId : b.periodId === activePeriodId,
                );
                const latestVisibleEndMs = relevantBars.length > 0 ? Math.max(...relevantBars.map((b: InternalPositionedDayBar) => b.end.getTime())) : null;
                const showBottomHandle = currentBarSelected && latestVisibleEndMs !== null && bar.end.getTime() === latestVisibleEndMs;

                const visibleDragRangeForEnd = latestVisibleEndMs !== null
                  ? isEditBar
                    ? (() => {
                        const [assetId, userName, indexText] = (bar.periodId ?? '').split('__');
                        const base = editPeriodsByAssetUser[assetId]?.[userName]?.[Number(indexText)];
                        return base ? { start: base.start, end: formatDateTime(new Date(latestVisibleEndMs)) } : undefined;
                      })()
                    : activePeriod?.requestedRange
                      ? { start: activePeriod.requestedRange.start, end: formatDateTime(new Date(latestVisibleEndMs)) }
                      : undefined
                  : undefined;

                const canMoveWholePeriod = isEditBar ? true : currentBarSelected && !!startVisible && !!activeRequestedStart && bar.start.getTime() === activeRequestedStart.getTime();

                return (
                  <div
                    key={bar.id}
                    className={`reservation-block draft ${currentBarSelected ? 'active' : ''} status-${bar.status ?? 'neutral'}`}
                    style={{ top: bar.top, height: bar.height, backgroundColor: bar.color, left: `calc(${bar.leftPct}% + 2px)`, width: `calc(${bar.widthPct}% - 4px)` }}
                    title={bar.tooltip}
                    onMouseDown={(e) => {
                      if (!bar.periodId) return;
                      if (isEditBar) {
                        const [assetId, userName, indexText] = bar.periodId.split('__');
                        setSelectedEditPeriod({ assetId, userName, index: Number(indexText) });
                        setAddForTarget(null);
                      } else {
                        setActivePeriodId(bar.periodId);
                      }
                      if ((mode === 'edit' || mode !== 'checkout') && canMoveWholePeriod) beginDrag('move', e, bar.periodId);
                    }}
                    onClick={(e) => {
                      e.stopPropagation();
                      if (!bar.periodId) return;
                  ...
                  >
                    {showTopHandle && mode !== 'checkout' && (
                      <div className="drag-handle top" onMouseDown={(e) => { if (!bar.periodId) return; beginDrag('resize-start', e, bar.periodId); }} />
                    )}
                    <div className="block-label">{bar.label}</div>
                    {showBottomHandle && mode !== 'view' && (
                      <div className="drag-handle bottom" onMouseDown={(e) => { if (!bar.periodId) return; beginDrag('resize-end', e, bar.periodId, visibleDragRangeForEnd); }} />
                    )}
                  </div>
                );
              })}
              ...
              {dayLayouts[dayIndex].existing.map((bar: InternalPositionedDayBar) => (
                <div
                  ...
                  title={bar.tooltip}
                >
                  <div className="block-label">{bar.showWarning ? '! ' : ''}{bar.label}</div>
                </div>
              ))}
            </div>
          ))}
      ...
      {omittedAssets.length > 0 && (
        <div className="calendar-message calendar-message-info">
          <div className="calendar-message-title">A few assets were left out of the current visual view to keep the calendar readable.</div>
          <div className="calendar-message-list">
            {omittedAssets.map((asset: Asset) => (
              <div key={asset.id} className="calendar-message-row"><span>{asset.name}</span><span>{asset.id}</span></div>
            ))}
          </div>
        </div>
      )}
      ...
      {selectionGroupLimitMessage && (
        <div className="calendar-message calendar-message-soft-warning">{selectionGroupLimitMessage}</div>
      )}
    </section>
  );
}
